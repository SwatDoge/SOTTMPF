...
execute at @e[type=armor_stand,tag=ak_bullet] as @e[type=armor_stand,tag=ak_bullet] positioned ~ ~1.65 ~ unless block ^ ^ ^1 air run summon shulker ^ ^ ^1 {Tags:["ak_impact"],NoGravity:1b,AttachFace:0,Color:15b,NoAI:1,Silent:1}
execute at @e[type=armor_stand,tag=ak_bullet] as @e[type=armor_stand,tag=ak_bullet] positioned ~ ~1.65 ~ unless block ^ ^ ^1 air run kill @e[tag=ak_bullet]
